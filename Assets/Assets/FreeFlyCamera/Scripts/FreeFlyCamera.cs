...
public class FreeFlyCamera : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (_enableMovement)
        {
            ...
            // Set the maximum distance to check for collisions
            float maxDistance = currentSpeed * _currentIncrease;

            // Cast a ray to detect obstacles within the maximum distance
            Ray ray = new Ray(transform.position, deltaPosition.normalized);
            RaycastHit hit;
            if (Physics.Raycast(ray, out hit, maxDistance, ~(1 << LayerMask.NameToLayer("Trigger"))))
            {
            ...
            {
                // Move the camera normally if there are no obstacles within the maximum distance
